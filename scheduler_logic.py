...
def next_recurrence_time(original: datetime.datetime, recurrence: str, last: datetime.datetime) -> datetime.datetime:
    ...
    if recurrence == 'once':
        ...
        # Простой способ: +1 месяц (может "упасть" на 28–31 число)
        year = last.year + (last.month // 12)
        month = (last.month % 12) + 1
        day = min(last.day, [31, 29 if year % 4 == 0 and not year % 100 == 0 or year % 400 == 0 else 28, 31, 30, 31, 30,
...
async def publish_message_and_reschedule(
    job_context,
    ...
    try:
        if photo_file_id:
            ...
            msg = await bot.send_message(chat_id, text, disable_notification=not notify)
        ...
        print(f"Ошибка публикации задачи {msg_id}: {e}")
        return

    # Перепланирование, если повтор
    if recurrence != 'once':
        current = datetime.datetime.fromisoformat(current_publish_at_str)
        next_time = next_recurrence_time(current, recurrence, current)
        if next_time:
            # Локализуем → UTC для хранения
            next_utc = local_tz.localize(next_time).astimezone(pytz.UTC).replace(tzinfo=None)
            next_iso = next_utc.isoformat()
            update_next_publish_time(msg_id, next_iso)
            # Перезапуск планировщика
            schedule_all_jobs(job_context.application.job_queue)
        else:
